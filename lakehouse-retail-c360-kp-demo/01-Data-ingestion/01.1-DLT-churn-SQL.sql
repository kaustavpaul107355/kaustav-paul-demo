...
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=984752964297111&notebook=%2F01-Data-ingestion%2F01.1-DLT-churn-SQL&demo_name=lakehouse-retail-c360&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-retail-c360%2F01-Data-ingestion%2F01.1-DLT-churn-SQL&version=1">
...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-churn" href="#joblist/pipelines/477cee36-36dd-4075-9bd1-dea15b20e7ea" target="_blank">Churn Delta Live Table pipeline</a> to see it in action.<br/>
...
-- MAGIC %python
-- MAGIC display(spark.read.json('/Volumes/kaustavpaul_demo/lakehouse/c360/users'))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming orders (json)
-- MAGIC %python
-- MAGIC display(spark.read.json('/Volumes/kaustavpaul_demo/lakehouse/c360/orders'))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming clickstream (csv)
-- MAGIC %python
-- MAGIC display(spark.read.csv('/Volumes/kaustavpaul_demo/lakehouse/c360/events', header=True))
...
AS SELECT * FROM cloud_files("/Volumes/kaustavpaul_demo/lakehouse/c360/events", "csv", map("cloudFiles.inferColumnTypes", "true"))
...
AS SELECT * FROM cloud_files("/Volumes/kaustavpaul_demo/lakehouse/c360/orders", "json")
...
AS SELECT * FROM cloud_files("/Volumes/kaustavpaul_demo/lakehouse/c360/users", "json", map("cloudFiles.inferColumnTypes", "true"))
...
-- MAGIC #                                                                 Model name         |        
-- MAGIC #                                                                     |              |        
-- MAGIC predict_churn_udf = mlflow.pyfunc.spark_udf(spark, "models:/kaustavpaul_demo.lakehouse.dbdemos_customer_churn@prod", "int")
...
-- MAGIC
-- MAGIC Open the <a dbdemos-pipeline-id="dlt-churn" href="#joblist/pipelines/477cee36-36dd-4075-9bd1-dea15b20e7ea" target="_blank">Churn Delta Live Table pipeline</a> and click on start to visualize your lineage and consume the new data incrementally!
...
-- MAGIC
-- MAGIC <a href="/sql/dashboards/944f5d86-ccc6-4fe4-a4e0-8fa11d03041a-dlt---retail-data-quality-stats" target="_blank">Data Quality Dashboard</a>
...
-- MAGIC
-- MAGIC <a href="/sql/dashboards/0fccee85-c44d-4538-a625-eea610a7efad" target="_blank">Open the DBSQL Dashboard</a>
