...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE kaustavpaul_demo.lakehouse.ml_churn_features AS SELECT * FROM kaustavpaul_demo.lakehouse.churn_features;
...
spark.createDataFrame(expectations, headers.split(",")) \
  ...
  .write.mode("overwrite").option("overwriteSchema", "true").saveAsTable("kaustavpaul_demo.lakehouse.dlt_expectations")

# COMMAND ----------

spark.sql(f'alter table kaustavpaul_demo.lakehouse.dlt_expectations set tblproperties (delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true)')
